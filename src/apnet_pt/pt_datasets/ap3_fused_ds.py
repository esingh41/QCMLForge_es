...
def ap3_fused_collate_update(batch):
    ...
    for i, data in enumerate(batch):
        ...
        if has_precomputed:
            E_classical_elst_list.append(data.E_classical_elst)
            E_classical_ind_list.append(data.E_classical_ind)
    ...
    if has_precomputed:
        batched_data.E_classical_elst = E_classical_elst
        batched_data.E_classical_ind = E_classical_ind
    ...
    def process(self):
        ...
        for i in range(len(RAs)):
            ...
            if self.dimer_prop_model is not None:
                ...
                temp_batch = ap2_fused_collate_update_no_target([data])
                with torch.no_grad():
                    ...
                    E_elst_summed = scatter_sum_compile(E_elst, temp_batch.dimer_ind, ndimer)
                    E_ind_summed = scatter_sum_compile(E_ind, temp_batch.dimer_ind, ndimer)
                    data.E_classical_elst = E_elst_summed[0].cpu().item()
                    data.E_classical_ind = E_ind_summed[0].cpu().item()
